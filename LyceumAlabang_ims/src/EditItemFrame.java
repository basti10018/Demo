...
public class EditItemFrame extends javax.swing.JFrame {
    ...
    private String prod_department;
        ...
        if (ans == JOptionPane.YES_OPTION) {
            if (checkFields()) {
                ...
                try {
                    ...
                    if (dt.editItem(prod_id,particularTF.getText(), quantity, unitsList.getSelectedItem().toString(), datePurchased, barcodeTF.getText(),
                            unitPrice, unitCost, statusList.getSelectedItem().toString(), departmentList.getSelectedItem().toString())) {
                        String transaction = "edit item";
    ...
    void getProduct(String id) throws ParseException {
        this.prod_id = id;
        dt.getProduct(id);

        this.particular = dt.prod_description.get(0);
        this.quantity = Integer.parseInt(dt.prod_quantity.get(0));
        this.barcode = dt.prod_barcode.get(0);
        this.prod_department = dt.prod_department.get(0);
        this.datePurchased = dt.prod_date.get(0);
        this.remarks = dt.prod_remarks.get(0);
        this.units = dt.prod_units.get(0);
        this.unitPrice = Double.parseDouble(dt.prod_price.get(0));
        this.unitCost = Double.parseDouble(dt.prod_cost.get(0));
        java.util.Date date2 = new SimpleDateFormat("MM/dd/yyyy").parse(datePurchased);
        particularTF.setText(particular);
        dateChooser.setDate(date2);
        barcodeTF.setText(barcode);
        quantityTF.setText("" + quantity);
        unitPriceTF.setText("" + unitPrice);
        costPriceTF.setText("" + unitCost);
        show();
    ...
    boolean checkFields() {
      try{
          ...
          java.util.Date daten = new SimpleDateFormat("MM/dd/yyyy").parse(datePurchased);
          dateChooser.setDate(daten);
